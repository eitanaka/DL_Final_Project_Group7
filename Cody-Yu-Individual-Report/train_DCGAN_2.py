...
Purpose: Fine-Tuning DCGAN with LayerNormalizaiton

LN normalizes across the feature dimension. This means that LN computes the mean and variance used for normalization
...
#discriminator
class DiscriminatorLN(nn.Module):
    def __init__(self, ngpu):
        super(DiscriminatorLN, self).__init__()
        self.ngpu = ngpu
...
netD = DiscriminatorLN(ngpu).to(device)
...
def save_best_model(netG, netD, epoch):
    torch.save(netG.state_dict(), os.path.join(MODEL_DIR, 'DCGAN_netG_c3.pth'))
    torch.save(netD.state_dict(), os.path.join(MODEL_DIR, 'DCGAN_netD_c3.pth'))
