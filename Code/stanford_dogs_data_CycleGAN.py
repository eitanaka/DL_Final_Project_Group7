...
class DogImages(data.Dataset):
    ...
    def __init__(self, root, exclude_breed=None, include_only_breed=None, train=True, cropped=False, transform=None, target_transform=None, download=True):
        self.root = os.path.join(os.path.expanduser(root), 'StanfordDogs')
        self.exclude_breed = exclude_breed
        self.include_only_breed = include_only_breed
        self.train = train
        ...
        if exclude_breed:
            print(f"Excluding breed: {exclude_breed}")
            split = [item for item in split if exclude_breed not in item[0]]
        elif include_only_breed:
            print(f"Including only breed: {include_only_breed}")
            split = [item for item in split if include_only_breed in item[0]]
        ...
        if len(self._flat_breed_images) == 0:
            print("Warning: No images found for the specified filter criteria.")
...
def main():
    dataset = DogImages(root=DATA_DIR, exclude_breed=None, include_only_breed=None, train=True,
                        download=False, cropped=True, transform=transforms.ToTensor())
    ...
    if len(dataset) > 0:
        ...
        print("Dataset is empty or breed name is incorrect.")
